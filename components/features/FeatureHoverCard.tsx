...
export function FeatureHoverCard({ event, position }: FeatureHoverCardProps) {
  ...
  // Check if this is an AI Platform event with companion icon
  const isCompanionMilestone = event.iconUrl?.includes('bob-companion') && event.featureScreenshotUrl;

  // Check if we should show above or below based on viewport position
  const showAbove = position.y > window.innerHeight - ((isAICulture || isCompanionMilestone) ? 280 : 180);

  // Companion milestone events get a special hover card with the image
  if (isCompanionMilestone) {
    return (
      <motion.div
        ...
        className={cn(
          ...
          "bg-bob-purple-800/95 border-pink-500/30 backdrop-blur-sm"
        )}
        style={{
          left: position.x,
      ...
      >
        ...
        <div className="p-2 bg-gradient-to-r from-pink-500/20 to-orange-500/20">
          <h4 className="text-sm font-medium text-pink-200 text-center">
            {event.title}
          </h4>
        </div>
      </motion.div>
